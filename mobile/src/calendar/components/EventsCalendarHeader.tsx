...
// Sibling to CalendarHeader.tsx, pointed at the Events context instead of the
// HH one. Duplicated rather than parametrized so the working HH header is
// never at risk from an events-driven change.
export const EventsCalendarHeader: React.FC = () => {
  const { theme } = useTheme();
  const d = theme.derived;
  const { view, setView, selectedDate, setSelectedDate } = useEventsCalendar();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topRow}>
        ...
        <TouchableOpacity
          style={[styles.todayBtn, { borderColor: d.border, backgroundColor: d.surfaceAlt }]}
          onPress={() => setSelectedDate(new Date())}
        >
          <Text style={[styles.todayText, { color: isToday ? EVENT_COLOR : d.text }]}>
          ...
          {VIEWS.map((v) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[styles.tab, active && { backgroundColor: EVENT_COLOR }]}
              >
                <Text
                  style={[
                    styles.tabText,
                    { color: active ? '#fff' : d.textSecondary },
                  ]}
                >
                  {v.label}
                </Text>
...
const styles = StyleSheet.create({
  container: { paddingHorizontal: 12, paddingTop: 8, paddingBottom: 8, gap: 8 },
  ...
  todayBtn: {
    height: 36, paddingHorizontal: 16, borderRadius: 10, borderWidth: 1,
    alignItems: 'center', justifyContent: 'center',
  },
  todayText: { fontSize: 13, fontWeight: '700' },
  labelRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  label: { fontSize: 18, fontWeight: '800' },
  tabs: { flexDirection: 'row', backgroundColor: 'transparent', gap: 4 },
  tab: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 999 },
