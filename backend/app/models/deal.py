...
class Deal(Base, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_deals_original_price_non_negative",
        ),
        CheckConstraint(
            "deal_price IS NULL OR deal_price >= 0",
            name="ck_deals_deal_price_non_negative",
        ),
        CheckConstraint(
            "discount_percentage IS NULL OR (discount_percentage >= 0 AND discount_percentage <= 100)",
            name="ck_deals_discount_percentage_range",
        ),
    )
