...
export default function VenueList() {
  ...
  useEffect(() => {
    venuesApi.neighborhoods().then(setNeighborhoods).catch(() => {})
    venuesApi.venueTypes().then(setVenueTypes).catch(() => {})
  }, [])
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1>Bars</h1>
        <Link to="/venues/new" className="btn btn-primary">Add Bar</Link>
      </div>
      ...
      <div className="filters">
        ...
        />
        <select value={neighborhood} onChange={(e) => setNeighborhood(e.target.value)} className="filter-select">
          <option value="">All Neighborhoods</option>
          {neighborhoods.map((n) => <option key={n} value={n}>{n}</option>)}
        </select>
        <select value={venueType} onChange={(e) => setVenueType(e.target.value)} className="filter-select">
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th>Address</th>
                  <th className="sortable" onClick={() => handleSort('neighborhood')}>Neighborhood{sortIndicator('neighborhood')}</th>
                  <th>Type</th>
                  <th>Deals</th>
                  <th>Status</th>
                  <th>Actions</th>
              ...
              <tbody>
                {venues.map((v) => (
                  <tr key={v.id} className={!v.active ? 'row-inactive' : ''}>
                    ...
                    <td>{v.venue_type || '—'}</td>
                    <td>
                      <span className="deal-count">{v.active_deals_count}/{v.deals_count}</span>
                    </td>
                    <td>
                      <StatusToggle active={v.active} onToggle={() => handleToggle(v.id)} />
                    </td>
                    <td>
                      <Link to={`/venues/${v.id}/edit`} className="btn btn-small">Edit</Link>
                    </td>
                  </tr>
                ))}
                {venues.length === 0 && (
                  <tr><td colSpan={7} className="empty-row">No bars found</td></tr>
                )}
              </tbody>
            </table>
          </div>
