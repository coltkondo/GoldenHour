...
async def export_venues_csv(db: Session = Depends(get_db)):
    ...
    writer.writerow([
        ...
        "neighborhood", "venue_type", "google_place_id", "price_level", "rating",
        "verified", "active", "description", "created_at", "updated_at",
    ])
    for v in venues:
        writer.writerow([
            str(v.id), v.name, v.address, v.latitude, v.longitude,
            v.phone, v.website, v.neighborhood, v.venue_type,
            v.google_place_id, v.price_level, v.rating,
            v.verified, v.active, v.description,
...
async def export_deals_csv(db: Session = Depends(get_db)):
    ...
    writer = csv.writer(output)
    writer.writerow([
        "id", "venue_id", "title", "description", "category", "deal_type",
        "original_price", "deal_price", "discount_percentage", "items",
        "source", "verified", "active", "created_at", "updated_at",
    ])
    for d in deals:
        writer.writerow([
            str(d.id), str(d.venue_id), d.title, d.description,
            d.category, d.deal_type, d.original_price, d.deal_price,
            d.discount_percentage,
            "|".join(d.items) if d.items else "",
            d.source, d.verified, d.active,
