...
def upgrade():
    ...
    if "google_place_id" not in venue_cols:
        op.add_column(
            "venues", sa.Column("google_place_id", sa.String(255), nullable=True)
        )
    if "price_level" not in venue_cols:
        op.add_column("venues", sa.Column("price_level", sa.Integer(), nullable=True))
    if "rating" not in venue_cols:
        op.add_column("venues", sa.Column("rating", sa.Float(), nullable=True))

    existing_constraints = {
        c["name"] for c in inspector.get_unique_constraints("venues")
    }
    if "uq_venues_google_place_id" not in existing_constraints:
        op.create_unique_constraint(
            "uq_venues_google_place_id", "venues", ["google_place_id"]
        )

    # Deals: add source
    if "source" not in deal_cols:
        op.add_column(
            "deals",
