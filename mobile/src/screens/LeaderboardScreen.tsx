  ...
  return (
    <View style={[styles.container, { backgroundColor: d.background }]}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {loadError && (
          <View style={[styles.errorBanner, { backgroundColor: d.cardBackground, borderColor: d.error }]}>
            <Text style={[styles.errorText, { color: d.error }]}>Couldn't load leaderboard</Text>
          </View>
        )}

        {myEntry && (
          <View style={[styles.myRankBanner, { backgroundColor: d.cardBackground, borderColor: d.primary }]}>
            ...
            <View style={styles.myRankRight}>
              ...
              <Text style={[styles.myRankPts, { color: d.primary }]}>{myEntry.points_balance} pts</Text>
            </View>
          </View>
        )}
        ...
        {loading ? (
          ...
          <View style={[styles.emptyState, { backgroundColor: d.cardBackground, borderColor: d.border }]}>
            ...
            <Text style={[styles.emptySubtext, { color: d.textMuted }]}>Submit deals and earn points to appear here</Text>
          </View>
        ) : (
          entries.map((entry, idx) => {
            ...
            return (
              <View key={entry.user_id} style={[styles.entryCard, { backgroundColor: d.cardBackground, borderColor: isMe ? d.primary : d.border }]}>
                <View style={[styles.rankBadge, { backgroundColor: isMedal ? medalColor : d.filterInactive }]}>
                  {isMedal ? (
                ...
                <View style={[styles.avatar, { backgroundColor: d.filterInactive }]}>
                  <Text style={[styles.avatarText, { color: d.text }]}>{entry.username.charAt(0).toUpperCase()}</Text>
                </View>
                <View style={styles.entryInfo}>
                  <Text style={[styles.entryUsername, { color: d.text }]}>{entry.username}{isMe && <Text style={styles.youLabel}> (You)</Text>}</Text>
                  <Text style={[styles.entryMeta, { color: d.textMuted }]}>{entry.approved_count} approved</Text>
                </View>
                <View style={styles.entryPointsRow}>
                  <AppIcon name="points" size={12} color={isMedal ? medalColor : d.primary} />
                  <Text style={[styles.entryPoints, { color: isMedal ? medalColor : d.primary }]}>{entry.points_balance}</Text>
                </View>
              </View>
            );
          })
...
const styles = StyleSheet.create({
  ...
  myRankBanner: { borderRadius: 16, borderWidth: 1, padding: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 },
  ...
  entryCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 16, borderWidth: 1, padding: 12, marginBottom: 8, gap: 12 },
