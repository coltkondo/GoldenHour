...
export default function DealList() {
  ...
  useEffect(() => {
    venuesApi.list({ limit: 200 }).then((v) => setVenues(v as Venue[])).catch(() => {})
    dealsApi.categories().then(setCategories).catch(() => {})
    dealsApi.dealTypes().then(setDealTypes).catch(() => {})
  }, [])
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1>Deals</h1>
        <Link to="/deals/new" className="btn btn-primary">Add Deal</Link>
      </div>
      ...
      <div className="filters">
        ...
        />
        <select value={venueFilter} onChange={(e) => setVenueFilter(e.target.value)} className="filter-select">
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th className="sortable" onClick={() => handleSort('deal_type')}>Type{sortIndicator('deal_type')}</th>
                  <th>Price</th>
                  <th>Items</th>
                  <th>Status</th>
                  <th>Actions</th>
              ...
              <tbody>
                {deals.map((d) => (
                  <tr key={d.id} className={!d.active ? 'row-inactive' : ''}>
                    ...
                    <td><span className="badge badge-outline">{d.deal_type}</span></td>
                    <td>
                      {d.deal_price !== null ? (
                        <span>
                    ...
                    <td className="cell-items">
                      {d.items?.length ? d.items.join(', ') : '—'}
                    </td>
                    <td>
                      <StatusToggle active={d.active} onToggle={() => handleToggle(d.id)} />
                    </td>
                    <td>
                      <Link to={`/deals/${d.id}/edit`} className="btn btn-small">Edit</Link>
                    </td>
                  </tr>
                ))}
                {deals.length === 0 && (
                  <tr><td colSpan={8} className="empty-row">No deals found</td></tr>
                )}
              </tbody>
            </table>
          </div>
