...
- [x] [Remove-Rewards] - **P2** - Remove callout for rewards. Make more simplified ideally. Emphasize deals, events, and community. Fixed in `LoadingScreen.tsx`: feature row simplified from 4 items (Deals/Events/Points/Rewards) to 3 (Deals/Events/Community) — added a `UsersThree` icon (new import in `AppIcon.tsx`, no equivalent existed before) for Community. Subtext rewritten to drop "Earn points... unlock rewards" framing entirely. Rewards intentionally left out rather than swapped in as a 4th tile, matching `ECONOMY_SPEC.md`'s "not active yet" status and this session's broader rewards-deemphasis (privacy policy). Easy to add back as a 4th tile if wanted later — flagged to Andes, his call.
- [x] [Include-Logo] - **P2** - Have the logo we have take the center of the screen. Pendulum Swing into the center of the screen. Then have the additional icons come in below that. Fixed: replaced the "GLDNHR°" text wordmark with the actual `gldnhr-logo.png` asset (was never used anywhere in the app before — only the text logotype existed). Pendulum motion via `Animated.spring` on a rotation transform (tension 22 / friction 4 — low friction relative to tension is what makes a spring overshoot past its target and oscillate rather than ease straight in) instead of the previous simple fade+slide-in. Feature icons/tagline/CTA continue to sequence in after it via the existing `Animated.sequence`, unchanged.
- [x] [Remove-Location-Tag] - **P2** - It says State College in the splash screen. No need to include that. Fixed: fine print now just shows `v1.0.0`, no hardcoded market name.
...
- [x] [Venue-Pins-Size] - **P2** - Venue pins are massive. Need to shrink in size to be more visually appealing. Kind of ass right now. Fixed: pins are generated PNGs (`scripts/generate-pin-assets.js` → `sharp`), not resizable via a style prop — `react-native-maps`' `Marker.image` renders at the source file's native pixel dimensions. Shrunk the generator's base constants (`BUBBLE_W` 90→54, `BUBBLE_H` 74→44, `ARROW_H` 20→12, `RADIUS` 40→22, icon 36→22 — all other measurements are derived from these) and regenerated all 14 PNGs; actual output confirmed 90×94 → 54×56. `sharp` wasn't installed anywhere in the repo (added as a `mobile` devDependency — build-time only, never imported by app code).
...
- [x] [Default-View-To-Week] - **P2** - Change the default View for Events to be "week". Keep Happy Hour calendar default to be "day". Fixed: `EventsCalendarContext.tsx`'s `view` state back to `'week'`; `CalendarContext.tsx` (HH) left at `'day'`.
