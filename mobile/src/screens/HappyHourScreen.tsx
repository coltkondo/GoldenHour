...
export const HappyHourScreen = () => {
  ...
  const handleDirections = () => {
    Linking.openURL(`https://www.google.com/maps/dir/?api=1&destination=${venue.latitude},${venue.longitude}`);
  };
    ...
    <View style={[styles.container, { backgroundColor: d.background }]}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.heroMeta}>
              {venue.venue_type && (
                <View style={[styles.typePill, { backgroundColor: d.filterInactive }]}>
                  <Text style={[styles.typePillText, { color: d.textMuted }]}>{venue.venue_type}</Text>
                </View>
              )}
              {venue.verified && (
                <View style={[styles.verifiedPill, { backgroundColor: 'rgba(45,212,160,0.12)', borderColor: d.live }]}>
        ...
        <View style={styles.actionsRow}>
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: d.cardBackground, borderColor: d.border }]} onPress={handleDirections} activeOpacity={0.8}>
            <AppIcon name="directions" size={16} role="brand" />
            <Text style={[styles.actionButtonText, { color: d.text }]}>Directions</Text>
          </TouchableOpacity>

          {venue.phone && (
            <TouchableOpacity style={[styles.actionButton, { backgroundColor: d.cardBackground, borderColor: d.border }]} onPress={handleCall} activeOpacity={0.8}>
          ...
          {venue.website && (
            <TouchableOpacity style={[styles.actionButton, { backgroundColor: d.cardBackground, borderColor: d.border }]} onPress={handleWebsite} activeOpacity={0.8}>
              <AppIcon name="globe" size={16} role="brand" />
              <Text style={[styles.actionButtonText, { color: d.text }]}>Website</Text>
            </TouchableOpacity>
          )}

          <TouchableOpacity style={[styles.actionButton, { backgroundColor: d.cardBackground, borderColor: d.border }]} onPress={() => setReportModalVisible(true)} activeOpacity={0.8}>
        ...
        {loadError && (
          <View style={[styles.errorCard, { backgroundColor: d.cardBackground, borderColor: d.error }]}>
            <View style={styles.errorContent}>
              ...
              <Text style={[styles.errorSub, { color: d.textMuted }]}>Check your connection and try again</Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.body}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              {deals.length > 0 && (
                <View style={[styles.dealCountBadge, { backgroundColor: 'rgba(45,212,160,0.12)', borderColor: d.live }]}>
            ...
            {loading ? (
              ...
              deals.map((deal) => (
                <View key={deal.id} style={[styles.dealCard, { backgroundColor: d.cardBackground, borderColor: d.border }]}>
              ...
              <View style={[styles.emptyCard, { backgroundColor: d.cardBackground, borderColor: d.border }]}>
                ...
                <Text style={[styles.emptySubtext, { color: d.textMuted }]}>Check back during happy hour times</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.section}>
            ...
            {schedules.length > 0 ? (
              <View style={[styles.scheduleCard, { backgroundColor: d.cardBackground, borderColor: d.border }]}>
                  ...
                  return (
                    ...
                    >
                      <View style={styles.scheduleDayCol}>
                        <Text style={[styles.scheduleDayText, { color: isToday ? d.live : d.text }]}>{dayName.slice(0, 3)}</Text>
                        {isToday && (
                          <View style={[styles.todayBadge, { backgroundColor: 'rgba(45,212,160,0.12)', borderColor: d.live }]}>
                      ...
                      <View style={styles.scheduleTimesCol}>
                        {daySchedules.length > 0 ? (
                          daySchedules.map((s) => (
                            <Text key={s.id} style={[styles.scheduleTimeText, { color: isToday ? d.text : d.textMuted }]}>
                              {formatTime(s.start_time)} – {formatTime(s.end_time)}
                            </Text>
                          ))
                        ) : (
                          <Text style={[styles.scheduleClosedText, { color: d.textHint }]}>No happy hour</Text>
                        )}
                      </View>
                    </View>
                  );
...
const styles = StyleSheet.create({
  ...
  verifiedPill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 12, gap: 5, borderWidth: 1 },
  ...
  actionButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 12, borderWidth: 1, paddingVertical: 12, paddingHorizontal: 10, gap: 6 },
  actionButtonText: { fontSize: 12, fontWeight: '600' },

  errorCard: { borderRadius: 14, borderWidth: 1, padding: 14, marginHorizontal: 16, marginBottom: 16 },
  errorContent: { flex: 1 },
  ...
  dealCountBadge: { width: 22, height: 22, borderRadius: 11, justifyContent: 'center', alignItems: 'center', borderWidth: 1 },
  ...
  dealCard: { flexDirection: 'row', borderRadius: 16, borderWidth: 1, padding: 14, marginBottom: 10, alignItems: 'center' },
  dealThumb: { width: 44, height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  ...
  scheduleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 12, paddingHorizontal: 16 },
